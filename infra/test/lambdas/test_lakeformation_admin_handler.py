...
def index(monkeypatch):
    ...
    # ``setitem``, not ``sys.modules[...] = mod`` followed by ``delitem``: delitem
    # removes the entry IMMEDIATELY and restores it at teardown, which is exactly
    # backwards — the module was absent while ``exec_module`` ran and was left
    # behind in ``sys.modules`` afterwards. ``setitem`` registers it now and
    # removes it at teardown, so the module is visible to its own execution (as
    # dataclasses/pickle/relative imports would require) and leaks nothing.
...
def _written_admins(index):
    ...
    # Assert the call happened first: otherwise ``call_args`` is None and the
    # caller fails with ``AttributeError: 'NoneType' object has no attribute
    # 'kwargs'``, which hides the real problem (nothing was written at all).
    assert index._lf.put_data_lake_settings.called, "PutDataLakeSettings was never called"
