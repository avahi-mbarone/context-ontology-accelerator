...
_MODULE_PATH = Path(__file__).resolve().parents[2] / "integ" / "detail_helpers.py"
...
def _load() -> ModuleType:
    spec = importlib.util.spec_from_file_location("integ_detail_helpers", _MODULE_PATH)
...
def test_federation_refs_are_read_from_database_details() -> None:
    ...
    details = _mod.database_details(_ORACLE_DETAIL)
...
def test_jdbc_configuration_is_read_from_database_details() -> None:
    ...
    jdbc = _mod.database_details(_ORACLE_DETAIL)["jdbcConfiguration"]
...
def test_database_details_missing_or_null_is_an_empty_dict() -> None:
    ...
    assert _mod.database_details({}) == {}
    assert _mod.database_details({"databaseDetails": None}) == {}
...
def test_scan_error_returns_empty_without_a_scan_job_id() -> None:
    ...
    assert _mod.source_scan_error("https://api.example.com", {}, "ns", "src", {}) == ""
