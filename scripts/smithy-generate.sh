...
OPENAPI_SPEC="$GENERATED_DIR/openapi/openapi/ControlPlaneService.openapi.json"
if [ ! -f "$OPENAPI_SPEC" ]; then
    echo "✗ ControlPlaneService OpenAPI spec not found: $OPENAPI_SPEC"
    exit 1
fi
...
PYSERVER_PKG="$GENERATED_DIR/control-plane-python-server/coa_control_plane_server"
grep '^from coa_data_layer_server\.models\.' "$PYSERVER_PKG/__init__.py" \
    > "$PYSERVER_PKG/__init__.py.tmp" || true
echo "# Auto-generated — models only (see smithy-generate.sh)" > "$PYSERVER_PKG/__init__.py"
cat "$PYSERVER_PKG/__init__.py.tmp" >> "$PYSERVER_PKG/__init__.py" 2>/dev/null || true
rm -f "$PYSERVER_PKG/__init__.py.tmp"

# ── Write pyproject.toml for the Python server models ────────────────────────
